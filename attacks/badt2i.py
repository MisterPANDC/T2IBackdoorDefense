...
def main():
    ...
    def add_target(batch, ):
        ...
        ## Original + padding
        id_2 = torch.cat((batch["input_ids"][:, :-1][:, :76], Trigger_ids[:, -1:], Trigger_ids[:, -1:]), dim=1)
        # print(id_2.shape)
        # print("----")
        # print(batch["input_ids"][:, :-1][:, :76].shape)
        # print(batch["input_ids"][:, 1:-1][:, :78 - (Trigger_ids.shape[1])].shape)
