...
def view_images(images, num_rows=1, offset_ratio=0.02, path='visualization.png'):
...
def text2image_ldm_stable(
    ...
    # print("embedding size: ", text_embeddings.shape)
    max_length = text_input.input_ids.shape[-1]
    uncond_input = model.tokenizer(
        [""] * batch_size, padding="max_length", max_length=max_length, return_tensors="pt"
    )
    uncond_embeddings = model.text_encoder(uncond_input.input_ids.to(model.device))[0]
    # print("uncond embedding size: ", uncond_embeddings.shape)
...
@torch.no_grad()
def text2image_ldm_stable_edit(
    model,
    prompt: List[str],
    replace,
    pos,
    ...
    replace_embeddings = model.text_encoder(replace_input.input_ids.to(model.device))[0]
    # print("embedding size: ", text_embeddings.shape, replace_embeddings.shape)
    # print(text_input.input_ids[0][:10], replace_input.input_ids[0][:10])

    for i in range(len(pos)):
        text_embeddings[:, pos[i]] = replace_embeddings[:, replace_pos[i]]
